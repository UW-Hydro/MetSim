...
def main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:],"hi:p:d:s:f:o:u:a:b:",
                                    ...
                                    "domainfile=","statefile_initial=",
                                    "infile_prefix=","outfile_prefix=",
                                    "outfile_directory=","startdate",
                                    "enddate="])
    ...
    forcing_file = infile_directory + '/' + infile_prefix + "_%04d%02d" % (first_year,first_month) + '.nc'
    ...
    while year <= last_year:
        for month_i in range(12):
            ...
            if month_current < 12:
                enddate = pd.to_datetime(("%04d%02d%02d" % (year_current,month_current + 1,1)),format='%Y%m%d') - pd.Timedelta('1 days')
            else:
                enddate = pd.to_datetime(("%04d%02d%02d" % (year_current + 1,1,1)),format='%Y%m%d') - pd.Timedelta('1 days')
            ...
            if year == first_year and month_i == 0:
                ...
                params["stop"] = enddate
                print('Running forcings from start and stop dates:',startdate,enddate)
                ms = MetSim(params)
                ms.run()

                # delete any previous temporary state files created in wrapper script and save the current
                try:
                ...
                forcing_filename = (infile_prefix + "_%04d%02d" % (year_current,month_current) + '.nc')
            ...
            if not spinup:
                ...
                if month_i == 0:
                    ...
                    forcing_concat = xr.concat([forcing_concat,forcing_current], dim='time')
            ...
            date_i += 1
            print('Completed forcings from start and stop dates:', startdate, enddate)
        if spinup:
            ...
            outfile = (outfile_directory + '/' + outfile_prefix + "%04d_" % (year) + 'total.nc')
